...
\section{Proposed Methodology for Cybersecurity Prioritization in the Computing Continuum}
...
\subsection{End-to-End Methodological Flow}

The methodology follows a structured pipeline:

\begin{enumerate}
    \item Capabilities are defined as atomic inputs mapped to the architectural framework
    \item Capabilities are evaluated through the 4-dimensional filtering logic
    \item Selected capabilities are aggregated into priorities
    \item Priorities evolve across phases through controlled transitions
    \item The executive summary synthesizes the outcomes into strategic guidance
\end{enumerate}
...
\subsection{Capabilities as Structured Inputs}

Capabilities constitute the primary analytical units of the roadmap and are directly derived from the architectural framework (Fig.~\ref{fig:security_framework_verticals_boxes}). Each capability represents a discrete, actionable function required to ensure cybersecurity and cyber-resilience across the Cloud--Edge--IoT continuum.

Each capability is defined along three axes:

\begin{itemize}
    \item \textbf{Layer Mapping}: Association to a specific layer (device, edge, cloud, application)
    \item \textbf{Cross-Layer Scope}: Identification of dependencies across layers (e.g., identity, trust, orchestration)
    \item \textbf{Functional Intent}: The security objective it fulfills (e.g., confidentiality, integrity, availability, sovereignty)
\end{itemize}

Capabilities are treated as \textbf{atomic inputs} to the prioritization methodology and are evaluated independently before aggregation into higher-level priorities.


\subsection{4-Dimensional Transition Filtering Logic -- from capabilities to Priorities}

To ensure the roadmap provides actionable guidance for European R\&I funding (Horizon Europe) and strategic investment, capabilities are prioritized using a structured 4-filter logic that identifies the ``Critical Path'' items requiring immediate resource allocation during Phase~1 (0--5 years). This logic acts as a \textbf{decision function} that evaluates each capability against four dimensions:

\begin{enumerate}
     \item \textbf{Regulatory Urgency -- Compliance Filter (The ``Must-Do''  Filter):} Alignment with imminent EU regulatory requirements (e.g., NIS2, CRA, AI Act)
    \begin{itemize}
    ...
    \item \textbf{Foundational Dependency -- Architecture Filter (The ``Blocker''  Filter):} Degree to which the capability acts as a prerequisite for other capabilities
    \begin{itemize}
    ...
    \end{itemize}
       ...
       \item \textbf{The Sovereignty Impact / Strategic Sovereignty Risk -- Resilience Filter (The ``Extinction''  Filter):} Contribution to European digital sovereignty and strategic autonomy
    ...
    \end{itemize}   
    \item \textbf{Market Feasibility \& Ecosystem Alignment -- Adoption Filter (The ``Simpl/SME'' Filter):} Technical maturity, implementation readiness, and deployment constraints
    \begin{itemize}
...
\begin{itemize}
    \item Advances to the next phase
    \item Remains in the current phase
    \item Is deprioritized or deferred
\end{itemize}
...
\subsubsection*{Transition Definition and Logic}

Within this methodology, a \textbf{transition} represents the structured transformation of outputs between stages of the pipeline. Transitions are governed by explicit mapping rules rather than narrative interpretation:

\begin{itemize}
    \item \textbf{Capability $\rightarrow$ Priority Transition (Stage 1 $\rightarrow$ Stage 2):} Driven by the 4-filter logic, selecting and ranking capabilities.

    \item \textbf{Priority $\rightarrow$ Profile Transition (Stage 2 $\rightarrow$ Stage 3):} Each selected priority is expanded into a full 7-dimensional analytical structure.

    \item \textbf{Profile $\rightarrow$ Matrix Transition (Stage 3 $\rightarrow$ Stage 4):} Selected dimensions (Baseline, Activities, Enablers) are systematically mapped into matrix indicators (ODP, SSL, Operational State, Critical Enabler).

    \item \textbf{Matrix $\rightarrow$ Strategy Transition (Stage 4 $\rightarrow$ Stage 5):} Cross-priority aggregation identifies shared enablers, systemic risks, and phase-level strategic posture.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Baseline \& Target Objective (The Trajectory):}\\
    ...
    \textbf{Guiding Question:} Where do we stand today, and what exactly does success look like?\\
...
\end{itemize}
    ...
    \item \textbf{Primary Drivers (The ``Push'' Factors):}\\
    ...
    \textbf{Guiding Question:} Why must we prioritize building this capability right now?\\
    ...
    \textbf{Guiding Question:} Which specific actions provide the greatest leap in European sovereignty and security?\\
    \textbf{Output:} A shortlist of actionable R\&I and deployment interventions.

    \item \textbf{Catalysts \& Enablers (The ``Pull'' Factors \& EU Support):}\\
    ...
    \textbf{Guiding Question:} What specific EU funding, standards, and ecosystem support do we need?\\
    ...
    \item \textbf{Disruptors \& Systemic Risks (The Roadblocks):}\\
    ...
    \textbf{Guiding Question:} What structural vulnerabilities, market failures, or adversary breakthroughs could derail us?\\
    ...
    \item \textbf{Failsafe Mechanisms \& Resilience (The Safety Nets):}\\
    ...
    \textbf{Guiding Question:} If this technology is compromised, how does the system survive and gracefully degrade?\\
    ...
    \item \textbf{Transversal Meta-Layer (The Policy Alignment):}\\
    ...
    \textbf{Guiding Question:} How does mastering this capability support Europe's goals of sovereignty, sustainability, and trust?\\
...
\subsection{Progression Matrix: Measuring Capability Evolution}
...
After priorities are profiled and their cross-phase progression is captured in the transition matrix, a final synthesis step converts detailed technical analysis into executive-level strategic guidance. The synthesis is non-normative and evidence-driven, relying exclusively on outputs generated in previous stages.
...
\begin{itemize}
    ...
    \item The \textbf{cross-phase evolution} recorded in the progression matrices (ODP, SSL, operational state, and critical enablers)
\end{itemize}

\subsubsection*{Synthesis Logic}
The synthesis step identifies:
\begin{itemize}
    \item \textbf{Common patterns} across priorities (e.g., repeated dependencies on hardware trust, PQC migration, interoperability)
    \item \textbf{Cross-cutting bottlenecks} that appear in multiple priorities and require coordinated EU action
    \item \textbf{Strategic inflection points} where delays in one capability cascade into multiple downstream priorities
...
The output is an executive summary that distills the roadmap into a concise set of critical strategic messages, a prioritized investment narrative across all three phases, and a policy-aligned explanation of why these priorities matter for sovereignty, resilience, and trust. This ensures full traceability from raw capabilities to strategic recommendations.

\subsection{Methodology Validation and Application}

To demonstrate the applicability of the proposed methodology, selected priorities---Hardware Roots of Trust (Priority~1) and PQC Agility (Priority~2)---are evaluated across the defined progression matrix in Section~\ref{sec:priorities}.

The validation examines three criteria:
\begin{itemize}
    \item \textbf{Consistency:} Prioritization outcomes are reproducible when the 4-filter logic is applied independently to the same capability set.
    \item \textbf{Traceability:} Each strategic recommendation in the executive summary can be traced back to a specific capability, filter score, profile dimension, and matrix cell.
    \item \textbf{Regulatory alignment:} Priorities and their phase timelines are cross-checked against enforcement deadlines in NIS2, CRA, and the AI Act.
\end{itemize}

The evaluation confirms that the methodology enables systematic identification of critical-path capabilities, structured transition planning across phases, and evidence-based policy and investment guidance. Limitations, including the reliance on qualitative filter scoring and the absence of quantitative validation data, are discussed in Section~\ref{sec:discussion}.
...
\begin{itemize}
    \item Consistency of prioritization outcomes using the 4-filter logic
    \item Traceability from capability definition to strategic recommendation
    \item Alignment with EU regulatory frameworks, including NIS2, CRA, and the AI Act
\end{itemize}

The results confirm that the methodology enables:
\begin{itemize}
    \item Systematic identification of critical-path capabilities
    \item Structured transition planning across phases
    \item Evidence-based policy and investment guidance
\end{itemize}

The methodology combines a pipeline-oriented transformation model with a multi-dimensional analytical framework, ensuring both structural traceability and depth of evaluation.

\subsection{End-to-End Methodological Flow - summary}
...
To determine which capabilities advance across implementation phases, a structured filtering logic is applied. This logic acts as a \textbf{decision function} that evaluates each capability against four dimensions:
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Logic:} This prioritizes areas where the EU currently has 100\% foreign dependency or where a technical breakthrough (e.g., \textit{Q-Day} Quantum threat) would render existing defenses obsolete.
    \end{itemize}   
    \item \textbf{Market Feasibility \& Ecosystem Alignement -- Adoption Filter (The ``Simpl/SME''  Filter):}Technical maturity, implementation readiness, and deployment constraints
    \begin{itemize}
        \item \textbf{Question:} Does this align with existing EU heavy-investments (e.g., Simpl \cite{cloud_edge_roadmap}) and is it adoptable by SMEs?
        \item \textbf{Logic:} Priority is given to capabilities that act as force-multipliers for existing infrastructure, ensuring economic viability.
    \end{itemize}
