...
\item Third, a consolidation of those capabilities into \emph{ten
strategic priorities} grouped in three priority clusters (Sovereign
Trusted Infrastructure; Cognitive Security and Autonomous Cyber
Defence; Federated Operational Sovereignty), each analysed through a
\emph{seven-dimension template} and projected onto a three-phase
transition matrix over 2026--2040 (Section~\ref{sec:clusters}).
\item Fourth, a set of implementation-oriented recommendations
derived from the cluster analysis and anchored in the current EU
regulatory and industrial landscape, identifying concrete
\emph{Phase-1 critical enablers} (e.g.\ EUCC ``High'' certification of
an EU-origin RISC-V secure element; OASC MIM6 progression; a
standardised adversarial robustness test suite for security AI) whose
absence or delivery determines whether the 2040 target state is
reachable (Section~\ref{sec:recs}).
\end{itemize}
...
\footnotesize 6 Security Layers + 4 Cross-Layers\\[3pt]
\normalsize
...
\begin{itemize}
    \item \textbf{(D1)~Baseline and Target Objective}, describing the current state of the priority cluster, the intended future state, and the strategic change required to move from one to the other;
    \item \textbf{(D2)~Primary Drivers}, identifying the threat and regulatory drivers that make the priority cluster urgent and non-deferrable;
    \item \textbf{(D3)~High-Impact Activities}, identifying the concrete research, innovation, standardisation, or deployment actions expected to advance the priority cluster, with each action assigned to a target phase;
...
The continuum is modelled as a layered reference architecture spanning four execution environments (HPC, Cloud, Edge, Devices) and organised into six \textit{Security Layers} forming the vertical stack and four \textit{Transversal Security Cross-Layers} serving as horizontal enablers, as illustrated in Figure~\ref{fig:framework}. These environments represent distinct but interconnected sites of computation and control: HPC provides large-scale scientific and AI training capacity; Cloud offers elastic service hosting and orchestration; Edge brings computation closer to users, industrial sites, telecom infrastructure, and data sources; and Devices include resource-constrained sensors, actuators, embedded systems, and IoT endpoints operating closest to the physical world.

The decomposition aligns with authoritative reference architectures, including ETSI MEC GS~003~\cite{etsimec003}, NIST SP~800-207 and SP~800-207A for Zero Trust \cite{rose2020zta,chandramouli2023zta}, NIST CSF 2.0~\cite{nistcsf2}, ISO/IEC 27400~\cite{iso27400}, and the ENISA baseline for IoT \cite{enisa2017iot}. It is also consistent with the distributed computing-continuum framing established in recent literature \cite{dustdar2023,bittencourt2018} and consolidated in the European industrial technology roadmap for the next-generation cloud-edge \cite{ecroadmap2024}.

The layers are analytically distinct but operationally interdependent: a gap at one layer can propagate to adjacent layers, as documented across the edge-computing security literature \cite{alwarafy2021,ranaweera2021,roman2018}. The model provides the shared vocabulary used throughout the paper and makes cross-layer dependencies explicit, allowing the priority clusters and transition matrices in Section~\ref{sec:clusters} to be analysed as an interconnected portfolio rather than as isolated priorities. The EU cybersecurity certification framework \cite{eucsa} and its EUCC operationalisation~\cite{commreg2024eucc} provide the regulatory anchoring for several of the capabilities defined below.

The remainder of this section introduces each layer and cross-layer by first explaining its purpose within the security framework and then listing the capabilities associated with it. Table~\ref{tab:capabilities} consolidates these capabilities, their layer or cross-layer codes, and their principal references. Each capability is treated as an atomic analytical unit: the ten strategic priorities analysed in Section~\ref{sec:clusters} are derived by aggregating and translating these capabilities into actionable strategic needs.
...
The six Security Layers define the vertical stack of the continuum, moving from physical and silicon-level trust foundations to software, applications, identity and orchestration, data protection, and network infrastructure. They are presented from the lowest trust anchors to the communication fabric because weaknesses at lower layers can undermine the guarantees provided by higher layers, while higher-layer controls are needed to operationalise security, governance, and resilience across distributed environments.

\subsubsection{Layer 1: Hardware-Centric Security}
\label{sec:L1}

Layer~1 addresses the physical and silicon-level trust foundations of the continuum, providing the verifiable anchors on which higher-layer security assurances depend. It covers hardware roots of trust, secure boot, remote attestation, Trusted Platform Modules, Physical Unclonable Functions, and physical-layer detection primitives \cite{tpm2spec,sailer2004}. Without a verifiable silicon anchor, software-level attestation and cryptographic identity rest on assumptions that may be difficult to validate in distributed and physically exposed environments. Layer~1 is the principal locus of hardware sovereignty and the layer in which EUCC ``High'' certification \cite{commreg2024eucc} operates as a regulatory instrument of trust.
...
\textbf{Capability L1.2, Physical-Layer Threat Detection and Fingerprinting.} Exploitation of intrinsic wireless channel characteristics, such as Channel State Information, Carrier Frequency Offset, and RSSI, to support real-time device authentication and anomaly detection at the lowest communication layer. This capability complements upper-layer cryptographic protections and can provide additional identity assurance for resource-constrained devices that cannot reliably execute conventional authentication protocols.

\subsubsection{Layer 2: Software-Centric Security}
\label{sec:L2}

Layer~2 secures the software substrate on which continuum workloads execute, including hypervisors, container runtimes, distributed operating systems, and the pipelines that maintain them. Its central concerns are attack-surface reduction through OS and middleware hardening, and cryptographically verified, failure-resilient software distribution to field-deployed nodes where physical access for remediation is limited or infeasible \cite{ietfrfc9019,uptanestd}. Assurance at Layer~2 is necessary for trustworthy execution of the services and workloads built above it.

\textbf{Capability L2.1, OS and Middleware Hardening.} Security hardening of hypervisors (KVM/QEMU), container runtimes, and distributed operating systems, with emphasis on sovereign, lightweight Linux distributions optimised for edge resource constraints and reduced attack surface.

\textbf{Capability L2.2, Secure OTA Patch and Update Management.} Cryptographically verified, failure-resilient over-the-air update pipelines for field-deployed edge nodes such as smart grids, O-RAN radios, and industrial gateways, where physical access for remediation is limited or infeasible. The IETF SUIT architecture~\cite{ietfrfc9019} and the Uptane standard~\cite{uptanestd} provide the main reference frameworks.
...
\textbf{Capability L3.2, In-Band Network Telemetry and Deep Observability.} Embedding telemetry directly into packets or service-mesh flows traversing containerised workloads, enabling real-time security monitoring and anomaly detection without relying exclusively on dedicated out-of-band monitoring infrastructure.
...
\textbf{Capability L5.3, Signalling and Data-Plane Security.} Embedding trust into signalling, control-plane, and data-plane interactions, moving beyond best-effort packet forwarding toward cryptographically authenticated and integrity-protected exchanges.

\textbf{Capability L5.4, Distributed Reputation and Trust Networking.} Distributed reputation and trust mechanisms, potentially supported by Distributed Ledger Technology and smart contracts, enabling decentralised trust evaluation across heterogeneous providers without relying on a single governance point.

\textbf{Capability L5.5, Quantum Key Distribution for Critical Links.} Information-theoretically secure key exchange over optical fibre for selected point-to-point high-value links, such as government communications and critical infrastructure backbones, complementing algorithmic PQC for link-layer protection and governed by ETSI GS QKD~004~\cite{etsiqkd004} and GS QKD~014~\cite{etsiqkd014}.

\textbf{Capability L5.6, Secure Multi-Party Computation.} Mechanisms that allow multiple mutually distrusting parties to jointly compute functions over combined private inputs without revealing those inputs, extending confidential computing (Capability~L5.1) beyond single-party workloads to collaborative analytics across providers and jurisdictions.

\subsubsection{Layer 6: Network Infrastructure and Perimeter-Centric Security}
\label{sec:L6}

Layer~6 secures the communication fabric of the continuum, including 5G and 6G cores, disaggregated RAN, IT/OT gateways, far-edge compute, and non-terrestrial network segments. It aligns with 3GPP TS~33.501~\cite{tgpp33501}, O-RAN WG11 security specifications~\cite{oran2024}, Zero Trust for O-RAN~\cite{oranzt2024}, the SNS~JU SRIA~\cite{snsju2023}, and 3GPP NTN security~\cite{tgppntn}. Layer~6 is where cybersecurity intersects most directly with physical resilience, because failures or compromises in the communication fabric can cascade into operational disruption, safety hazards, or kinetic effects in critical infrastructures.
...
\textbf{Capability L6.2, Physical Resilience and Safety.} IT/OT gateway security for high-availability edge environments such as smart grids, transport systems, and manufacturing sites, reducing the risk that digital breaches cascade into operational disruption, kinetic damage, or safety hazards.

\textbf{Capability L6.3, Edge-Native Autonomous Guardians.} Lightweight, localised security control functions, including Security-as-a-Service aggregators, hosted directly at far-edge gateways or within Open-RAN Radio Intelligent Controllers to enable autonomous enforcement at the network periphery.

\textbf{Capability L6.4, Non-Terrestrial Network and Satellite Security.} Security requirements for integrating satellite and non-terrestrial network segments into 6G architectures for coverage continuity, backhaul resilience, and emergency operations, aligned with 3GPP NTN specifications~\cite{tgppntn} and the EU Space Programme.

\subsection{Transversal Security Cross-Layers}

The four Transversal Security Cross-Layers define horizontal security capabilities that span the vertical stack and address properties that no single layer can provide on its own: lifecycle integrity of software and AI components, operational response capacity, interoperability across providers and jurisdictions, and continuous regulatory assurance.

\subsubsection{Cross-Layer 1: Automated Security Processes and Cyber-Resilient AI Lifecycle}
\label{sec:CL1}

Cross-Layer~1 spans the lifecycle of software and AI component security, from secure development and integration to runtime supply-chain verification and continuous adversarial validation. It aligns with service-mesh DevSecOps patterns~\cite{chandramouli2022devsecops}, the NIST AI Risk Management Framework and its Generative AI Profile~\cite{nistairmf,nistaigenai}, the SLSA provenance framework~\cite{slsa2023}, and the adversarial robustness literature~\cite{madry2018}. Its role is to embed cyber-resilience into how software and AI components are designed, built, shipped, verified, and tested, rather than treating security assurance as a post-deployment activity.
...
\textbf{Capability CL1.2, Model Integrity and DataBOMs.} Protection of AI training and deployment pipelines against data poisoning, unauthorised model manipulation, and insufficient traceability of training data and model artefacts, aligned with the NIST AI Risk Management Framework~\cite{nistairmf} and its Generative AI Profile~\cite{nistaigenai}.
...
\textbf{Capability CL2.2, High-Precision Telemetry and Security Signalling.} High-fidelity telemetry and security signalling mechanisms that feed real-time threat intelligence and detection pipelines while managing bandwidth, latency, and alert-volume constraints.

\textbf{Capability CL2.3, Internet of Behaviour and Evidence Sharing.} Collection, correlation, and sharing of behavioural evidence across infrastructure components to detect anomalous patterns that may be invisible to purely signature-based defences.

\textbf{Capability CL2.4, AI-Native Analytics Integration (NWDAF and LLMSecOps).} Integration of cybersecurity telemetry with 5G NWDAF analytics~\cite{tgpp23288} and LLM-assisted security operations to support triage, correlation, investigation, and response.
...
\textbf{Capability CL2.6, Deception Technology and Active Defence.} Deployment of decoy assets, such as honeypots, honeynets, credential canaries, and fake API endpoints, to generate high-fidelity alerts based on adversary interaction and to support earlier detection of lateral movement or reconnaissance.
...
providers, infrastructures, and jurisdictions. It anchors federation in the Simpl open-source middleware for Common European Data Spaces~\cite{ecsimpl}, CAMARA Network APIs for pan-European compute federation~\cite{camara2025}, semantic interoperability through OASC Minimal Interoperability Mechanisms~\cite{oascmim5} and the ETSI NGSI-LD API~\cite{etsingsild}, and federated cyber ranges for shared validation infrastructure. Its role is to convert individually sovereign components into a collectively sovereign European trust fabric while avoiding new forms of vendor lock-in.

\textbf{Capability CL3.1, Heterogeneous Interoperability and Simpl Integration.} Alignment with the Simpl open-source smart middleware~\cite{ecsimpl} to support secure federated access, policy portability, and interoperability across Common European Data Spaces.

\textbf{Capability CL3.2, Neutral Host and Multi-Tenant Isolation.} Enforcement of Zero Trust edge operations across network partitions, tenants, and shared-infrastructure models, ensuring that infrastructure sharing does not imply implicit trust between operators or workloads.
...
\textbf{Capability CL3.5, Cyber Range and Digital Twin Federation.} Federation of national cyber ranges and digital-twin environments into shared EU-wide validation infrastructure for testing cross-provider security, resilience, and interoperability assumptions, aligned with the Cyber Solidarity Act.

\subsubsection{Cross-Layer 4: Compliance-by-Design, Agile Governance
and Automated Assurance}
\label{sec:CL4}

Cross-Layer~4 embeds regulatory compliance, governance, and assurance considerations into technical architecture. It operationalises continuous assurance for cloud services (EUCS, when adopted) and edge products (EUCC)~\cite{commregreg2024eucc,eucsa} through machine-readable control catalogues and assessment results using NIST OSCAL~\cite{oscal2024,iorga2024oscal}; cryptographic key management across multi-provider operations; privacy-preserving federated computation satisfying GDPR Article~25~\cite{eugdpr}; cyber risk quantification that translates technical posture into financial and operational risk language; and AI trustworthiness and algorithmic accountability for autonomous security decisions under AI Act Article~9~\cite{euaiact}, aligned with the NIST AI RMF~\cite{nistairmf}. Its role is to shift assurance from point-in-time audit towards continuous, evidence-based, and telemetry-supported verification.

\textbf{Capability CL4.1, Post-Quantum Readiness and Zero Trust Architectures.} Continuous Zero Trust verification and post-quantum readiness embedded into cloud-edge infrastructure, aligned with NIST SP~800-207~\cite{rose2020zta} and SP~800-207A~\cite{chandramouli2023zta}.
...
\textbf{Capability CL4.3, Multicloud Key Management as a Service.} Cryptographic key, policy, and lifecycle management across dynamic multi-provider computing-continuum operations.

\textbf{Capability CL4.4, Privacy-Preserving Federated Computation.} Cross-jurisdictional AI training and inference over distributed private datasets using privacy-preserving computation mechanisms, supporting GDPR Article~25~\cite{eugdpr} data-protection-by-design objectives.

\textbf{Capability CL4.6, Cyber Risk Quantification and Board-Level Reporting.} Translation of technical security posture into financial, operational, and strategic risk language for decision-makers.

\textbf{Capability CL4.7, AI Trustworthiness and Algorithmic Accountability.} Assurance that AI systems making or supporting autonomous security decisions remain robust, accountable, and auditable, in compliance with AI Act Article~9~\cite{euaiact} and aligned with the NIST AI RMF~\cite{nistairmf}.
