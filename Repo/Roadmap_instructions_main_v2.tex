...
\textbf{1.1 Hardware Roots of Trust:} Cryptographically verifiable supply chain trust (e.g., TPM attestation, secure enclaves) to ensure physical device integrity. This requires driving European open-hardware designs toward EUCC 'High' assurance level certification \cite{eucc} to mathematically guarantee resistance against state-level physical tampering at the vulnerable far-edge.


\subsubsection*{Layer 2: Software-Centric Security (Middleware/OS)}
\textbf{2.1 OS \& Middleware Hardening:} Securing hypervisors, container runtimes, and distributed operating systems operating across the edge-cloud continuum.

\subsubsection*{Layer 3: Application-Centric Security (Apps/Services)}
\textbf{3.1 Workload Security:} Securing containerized applications, microservices, and serverless functions executing dynamically across multi-provider nodes.

\subsubsection*{Layer 4: User \& Orchestration-Centric Security}
\textbf{4.1 Zero-Trust Orchestration:} Seamless policy enforcement across a multi-provider computing continuum, utilizing dynamic access controls.

\textbf{4.2 Federated Identity \& Access Management (IAM):} Cross-domain identity federation and verifiable credentials for machines, users, and automated workloads.

\subsubsection*{Layer 5: Data-Centric Security}
\textbf{5.1 End-to-end Confidential Computing:} Data-in-use security and Trusted Execution Environments (TEEs) spanning the entire Cloud $\rightarrow$ Edge $\rightarrow$ IoT data path.

\textbf{5.2 Post-Quantum \& Cryptographic Agility:} PQC standardization, hybrid crypto deployment, and dynamic crypto-agility to secure long-lived data against ``Harvest Now, Decrypt Later'' threats.

\subsubsection*{Layer 6: Network Infrastructure \& Perimeter-Centric Security}
\textbf{6.1 Telco-cloud \& O-RAN Security:} Hardening distributed 5G/6G cores, RAN components, and securing software-defined networking perimeters. Security controls and architectural blueprints must align with the Smart Networks and Services Joint Undertaking (SNS JU) Strategic Research and Innovation Agenda (SRIA) \cite{snsju} to ensure that upcoming European 6G infrastructures are sovereign and secure-by-design.

\textbf{6.2 Cross-Sector Physical Resilience \& Safety:} Adapting strict network controls and fail-safes for high-availability edge environments (e.g., Smart Grids, Software-Defined Vehicles, Industrial IoT) to ensure that cyber-breaches do not cascade into physical safety hazards or kinetic damage. Security at the far-edge must mathematically guarantee operational safety.
...
\begin{enumerate}
    ...
    \textit{Description:} Establishes the starting point of the capability today and defines the ultimate maturity state required by the end of the 15-year roadmap.
    \begin{itemize}
