...
\caption{Master Capability Transition Framework: Phase States, Gate Conditions, and SSL Bands across Four Analytical Dimensions. Gate condition rows state named, verifiable triggers for each phase transition derived from the Critical Enabler cells of the per-priority progression matrices. SSL bands track Strategic Sovereignty Level per phase. KPI anchors serve as cross-reference consistency anchors only.}
...
\newline\textit{2026 baseline}
\newline\sslbadge{sslredbg}{sslredtx}{SSL: High Foreign Dependency}
&
\cellhead{Fragmented consortia output; sovereign production capacity absent}
EU industrial R\&I activity exists --- Horizon~2020 Networks of Excellence
(CONCORDIA, SPARTA, CyberSec4Europe) and active Horizon~Europe
participation by NXP, Thales, Infineon, Nokia --- but outputs remain at
TRL~3--5 for sovereign alternatives. Industrial partners contribute to
research consortia without translating results into deployable sovereign
production: no EU-manufactured hardware root of trust at EUCC `High',
no edge-native TEE independent of Intel TDX or AMD SEV-SNP, no
lightweight PQC silicon from EU fabrication. ECCC, established 2021,
is still building investment coherence across fragmented national
programmes.
